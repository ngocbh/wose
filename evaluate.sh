...
then 
	${PYPY} tools/evaluations/evaluate_ranking.py --dict ${DICTIONARY} --rank ${RANKING_MULTI} --threshold 0.85 --outbase "evaluation_multi-words.txt" --outdir "experiment/"
	${PYPY} tools/evaluations/evaluate_ranking.py --dict ${DICTIONARY} --rank ${RANKING_UNIGRAMS} --threshold 0.85 --outbase "evaluation_unigrams.txt" --outdir "experiment/"
	${PYPY} tools/evaluations/evaluate_ranking.py --dict ${DICTIONARY} --rank ${RANKING_SALIENT} --threshold 0.85 --outbase "evaluation_salient.txt" --outdir "experiment/"
fi
