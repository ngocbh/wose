...
public:
    ...
    void train(const vector< vector<double> > &features, const vector<double> &results, int minNodeSize, int maxLevel = 18, vector<string> featureNames = vector<string>()) {
        ...
        //Mapping feature name to id ( những feature có feature name giống nhau thì mapping vào cùng một id )
        ...
        if (featureNames.size() != 0) {
            ...
            // nếu ko cung cấp feature name thì coi các feature name là như nhau . 
            // vector<int>(1,i) là constructor khởi tạo size = 1 và chứa giá trị i
            for (int i = 0; i < features[0].size(); ++ i) {
                featureGroups.push_back(vector<int>(1, i));
            }
        }

        //create tree by add root
        TreeNode root;
        root.level = 0;
