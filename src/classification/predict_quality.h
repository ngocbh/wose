...
void predictQuality(vector<Pattern> &patterns, vector<vector<double>> &features, vector<string> &featureNames)
{
    ...
    RANDOM_FEATURES = 4;
    RANDOM_POSITIONS = 4;
    K_OUT_OF_N = 100; // có vẻ là số lượng label lấy ra từ pool

    // cerr << "K = " << K_OUT_OF_N << endl;

    // fprintf(stderr, "[multi-word phrase] phrase quality estimation...\n");
    // Training input / training labeled / number of decision tree / min node size / max level of decision tree / name of feature
